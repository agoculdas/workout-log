...
export function Session() {
  ...
  const remainingSets = exercises.reduce(
    (sum, ex) => sum + Math.max(0, plannedSets(ex) - (detail.setsByExercise[ex.id]?.length ?? 0)),
    0,
  );
   ...
   * (carry-forward), and finally the progression suggestion.
   */
  function valuesFor(setIndex: number): Draft & { stored?: SetLog } {
    const stored = loggedSets.find((s) => s.setIndex === setIndex);
    const draft = drafts[keyFor(setIndex)];
    if (draft) return { ...draft, stored };
    if (stored) return { load: stored.load, reps: stored.reps, stored };

    const prior = loggedSets
      .filter((s) => s.setIndex < setIndex)
      .sort((a, b) => a.setIndex - b.setIndex)
      .pop();
    if (prior) return { load: prior.load, reps: prior.reps };

    return {
      load: suggestion && suggestion.load > 0 ? suggestion.load : null,
      reps: suggestion && suggestion.reps > 0 ? suggestion.reps : null,
    };
  }
  ...
  function patchDraft(setIndex: number, patch: Partial<Draft>): void {
    ...
    setDrafts((prev) => ({
      ...prev,
      [keyFor(setIndex)]: { load: current.load, reps: current.reps, ...patch },
    }));
  }
  ...
  async function handleDone(setIndex: number): Promise<void> {
    ...
    if (exercise.type !== 'conditioning') {
      ...
      timer.start(exercise.type === 'primary' ? settings.restPrimary : settings.restAccessory);
    }
  }
  ...
  async function handleUndo(setIndex: number): Promise<void> {
    ...
    setDrafts((prev) => ({
      ...prev,
      [keyFor(setIndex)]: { load: stored.load, reps: stored.reps },
    }));
  ...
  const rowCount = Math.max(plannedSets(exercise), loggedSets.length);
    ...
    <div>
      ...
      >
        ...
        <div className="mt-5 mb-4 flex flex-wrap items-center justify-center gap-1.5">
          {exercises.map((ex, i) => {
            const complete = (detail.setsByExercise[ex.id]?.length ?? 0) >= plannedSets(ex);
        ...
        {suggestion?.progressed ? (
          <div className="mb-3 rounded-xl border border-accent/40 bg-accent/10 px-3 py-2 text-sm font-medium text-accent">
            ▲ +{formatNumber(exercise.increment)} kg suggested
          </div>
        ) : null}
      ...
      >
        <ul className="flex flex-col gap-1">
          {exercises.map((ex, i) => {
            const done = detail.setsByExercise[ex.id]?.length ?? 0;
